...
class Redshift(SQLAdapter):
    ...
    def get_columns(
        ...
        query = f"SELECT d.column FROM pg_table_def AS d WHERE d.tablename = '{name}'"
        if schema is not None:
            query += f" AND d.schemaname = '{schema}'"
    ...
    def vacuum(
        ...
        self.execute(f'VACUUM FULL {table} TO 99 PERCENT', autocommit=True)
        self.execute(f'ANALYZE {table}', autocommit=True)
