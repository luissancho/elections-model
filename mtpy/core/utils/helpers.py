...
def infer_mtype(x: pd.Series) -> str | None:
    ...
    if d.str.lower().map(bin_map).isin([True, False]).all():
        return 'bin'

    if is_timeseries(d.tolist()):
        d = pd.to_datetime(d, errors='coerce').dt.floor('s', ambiguous=False)
        if d.dt.time.max() == d.dt.time.min():
            return 'dtd'
        else:
            return 'dts'

    if is_numeric(d.tolist()):
        d = pd.to_numeric(d, errors='coerce').astype(float)
        if d.apply(float.is_integer).all():
            return 'int'
        else:
            return 'num'

    if d.str.startswith(('{', '[')).all() and d.str.endswith(('}', ']')).all():
        return 'obj'

    if d.unique().shape[0] < 8:
        return 'cat'
...
def infer_meta(
    ...
    exclude = exclude or []
    meta = {}
    columns = [col for col in x.columns if col not in exclude]
    ...
    for col in columns:
        ...
        if detailed:
            ...
            if mtype in ['str', 'obj', 'cat']:
                mlen = int(d.str.len().max())
                mtype = [mtype, mlen]
            elif mtype == 'int':
                d = pd.to_numeric(d, errors='coerce')
                mlen = int(math.log(d.max(), 256)) + 1
                mtype = [mtype, mlen]
