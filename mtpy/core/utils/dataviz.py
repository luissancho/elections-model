...
def get_color(
    color: str | tuple,
    alpha: Optional[Literal['alpha', 'light', 'bold', 'bolder'] | float] = None,
    mode: Literal['hex', 'rgb', 'rgba', 'hsv'] = 'hex'
) -> str | tuple:
    """
    Parse color and return it in the specified mode.

    Parameters
    ----------
    color : str | tuple
        Color to be parsed.
    alpha : {'alpha', 'light', 'bold', 'bolder'} | float, optional
        Alpha value to be applied to the color.
    mode : {'hex', 'rgb', 'rgba', 'hsv'}, default 'hex'
        The format of the color to be returned.

    Returns
    -------
    str or tuple
        Color in the specified mode.
    """
    if '-' in color:
    ...
    if alpha is not None:
        color = build_color_seq_map(color)(alpha / 2)[:3]

    if isinstance(color, tuple) and (np.array(color) > 1.).any():
        color = tuple(np.array(color) / 255.)

    if mode == 'rgb':
        return tuple(int(round(c * 255)) for c in mcolors.to_rgb(color))
    elif mode == 'rgba':
        return tuple(mcolors.to_rgb(color)) + (1.,)
    elif mode == 'hex':
        return mcolors.to_hex(color)
    elif mode == 'hsv':
        return tuple(mcolors.rgb_to_hsv(mcolors.to_rgb(color)))
    else:
        raise ValueError(f'Invalid mode: {mode}')


def build_color_cat_map(
    colors: list[str | tuple],
    n: Optional[int] = None,
    gamma: float = 1.0
) -> mcolors.LinearSegmentedColormap:
    """
    Build a categorical color map.

    Parameters
    ----------
    colors : list-like of colors
        Colors to be parsed and transformed.
    n : int, optional
        Number of colors to be returned.
    gamma : float, default 1.0
        Gamma value to be applied to the color map.
...
def build_color_seq_map(
    ...
    """
    Build a sequential color map.

    Parameters
    ----------
    color : str | tuple
        Color to be parsed and transformed.
    n : int, default 50
        Number of colors to be returned.
    beta : float, default 0.05
        Beta value to be applied to the color map.
    gamma : float, default 1.0
        Gamma value to be applied to the color map.
...
def build_color_map(
    cm: list[str | tuple] | str | tuple | mcolors.Colormap,
    rtype: Literal['plt', 'cv2'] = 'plt',
    **kwargs
) -> mcolors.Colormap | np.ndarray:
    cmap = None

    if isinstance(cm, mcolors.Colormap):
        cmap = cm
    elif isinstance(cm, list):
        if len(cm) > 3:
            cmap = build_color_cat_map(cm, **kwargs)
        elif len(cm) > 1:
            cmap = build_color_div_map(cm, **kwargs)
        elif len(cm) > 0:
            cmap = build_color_seq_map(cm[0], **kwargs)
    elif isinstance(cm, (str, tuple)):
        if cm in list(plt.colormaps):
            cmap = plt.get_cmap(cm)
        else:
            cmap = build_color_seq_map(cm, **kwargs)

    if cmap is not None and rtype == 'cv2':
        cmap = plt.cm.ScalarMappable(cmap=cmap).to_rgba(
            np.linspace(0, 1, 256),
            bytes=True
        )[:, 2::-1].reshape(256, 1, 3)
...
def get_color_map(
    cm: Optional[list[str | tuple] | dict[str, str | tuple] | str | None] = None,
    keys: Optional[list[str] | int] = None,
    mode: Literal['hex', 'rgb', 'rgba', 'hsv'] = 'hex',
    rtype: Optional[Literal['list', 'dict', 'plt', 'cv2']] = None,
    **kwargs
) -> list[str | tuple] | dict[str, str | tuple] | mcolors.LinearSegmentedColormap:
    """
    ...
    mode : {'hex', 'rgb', 'rgba', 'hsv'}, default 'hex'
        The format of the colors to be returned.
    rtype : {'list', 'dict', 'plt', 'cv2'}, default 'list'
        ...
        - ``cv2`` :
            A ``cv2.Colormap`` built with the resulting colors list.
    **kwargs
    ...
    cmap = [get_color(c, mode=mode) for c in cmap]
    if rtype == 'dict':
...
def get_text_color(
    bg_color: str | tuple,
    cm: Optional[list[str | tuple]] = None,
    ...
    ----------
    bg_color : str | tuple
        Background color to be parsed and transformed.
    cm : list-like of colors, optional
        Colors to be parsed and transformed.
    mode : {'hex'}, default 'hex'
        The format of the colors to be returned.
    min_lum : float, default 0.408
        Minimum luminance to be considered for the text color.
    ...
    color = cm[0] if lum > min_lum else cm[1]

    return get_color(color, mode=mode)
...
def create_figure(ax=None, n=None, **kwargs):
    ...
    sharex = kwargs.get('sharex', kwargs.get('share', False))
    sharey = kwargs.get('sharey', kwargs.get('share', False))
...
def save_figure(fig=None, path=None):
    if fig is not None and path is not None:
        with tempfile.TemporaryFile() as tf:
            fig.savefig(tf, bbox_inches='tight', pad_inches=0.0)
...
def get_df_styler(
    data: pd.DataFrame,
    meta: Optional[dict[str, str] | str] = None,
...
def plot_series(data, ax=None, show=True, path=None, **kwargs):
    ...
    _plt_params = dict(
        ...
        grid=True, frame=True, disp=None, vlines=None, hlines=None,
...
def plot_histogram(data, ax=None, show=True, path=None, **kwargs):
    ...
    _plt_params = dict(
        ...
        xlim=None, xmin=None, xmax=None, xticks=None, xfmt=None, ylim=None, ymin=None, ymax=None, yticks=None,
    ...
    if isinstance(data, pd.DataFrame):
        return plot_histogram_grid(data, show=show, path=path, **kwargs)
    ...
    if plt_params['xfmt'] is not None:
        xticklabels = [plt_params['xfmt'].format(format_number(t)) for t in ax.get_xticks()]
    else:
...
def plot_scores(data, ax=None, show=True, path=None, **kwargs):
    ...
    if plt_params['sort']:
        df = df.sort_values(col_main, ascending=False)

    s = df[col_main].fillna(0).values
    idx = df.index
    err = df[cols_err].fillna(0).clip(lower=0).T.values if cols_err is not None else None
    cm = df['cm'].values
...
def plot_image(image, ax=None, show=True, path=None, **kwargs):
    _plt_params = dict(
        cmap='gray', title=None, note=None
    )
    _fig_params = dict(
        figsize=None
    )

    if isinstance(image, (list, tuple, dict)):
        return plot_image_grid(image, show=show, path=path, **kwargs)
    ...
    ax.imshow(image, cmap=plt_params['cmap'])
...
def plot_heatmap(data, ax=None, show=True, path=None, **kwargs):
    _plt_params = dict(
        cmap='gray', vmin=None, vmax=None, cbar=False, clabels=None, title=None, note=None
    )
    _fig_params = dict(
        figsize=None
    )
    ...
    if isinstance(data, (list, tuple, dict)):
        return plot_heatmap_grid(data, show=show, path=path, **kwargs)
    ...
    cmap = plt_params['cmap']
    if not isinstance(cmap, mcolors.Colormap):
        if is_array(cmap):
            if len(cmap) == 2:
                cmap = build_color_seq_map(cmap)
            else:
                cmap = build_color_cat_map(cmap)
        elif '#' in cmap or cmap in colors:
            cmap = build_color_seq_map(get_color(cmap))
    ...
    m = ax.imshow(heatmap, cmap=cmap, aspect='equal', vmin=vmin, vmax=vmax)

    ax.set_xticks([])
    ax.set_yticks([])

    if plt_params['cbar']:
        labels = plt_params['clabels']
        if labels is not None:
            ticks = np.linspace(vmin, vmax, len(labels) + 1)[:-1] + (1 / (2 * len(labels))) - .01
            cb = ax.figure.colorbar(m, ax=ax, pad=.01, location='right', ticks=ticks)
            cb.ax.set_yticklabels(labels)
        else:
            cb = ax.figure.colorbar(m, ax=ax, pad=.01, location='right')
...
def plot_colors(colors, ax=None, show=True, path=None, **kwargs):
    _plt_params = dict(
        labels=None, title=None, note=None
    )
    _fig_params = dict(
        n_cols=1, figsize=(6.4, .8)
    )

    plt_params = get_params(_plt_params, None, 'plt_params', **kwargs)
    fig_params = get_params(_fig_params, None, 'fig_params', **kwargs)

    labels = plt_params['labels']
    if labels is None:
        labels = [None] * len(colors)
    else:
        if not isinstance(labels, list):
            labels = [labels]
        labels = [labels[i] if i < len(labels) else None for i in range(len(colors))]

    if not isinstance(colors, list):
        colors = [colors]

    if all(isinstance(c, (str, tuple)) for c in colors):
        colors = [mcolors.ListedColormap([
            get_color(c, mode='hex') for c in colors
        ], name=None)]
        labels = [labels]

    if fig_params['figsize'] is None:
        fig_params['figsize'] = (6.4, .8 * len(colors))

    fig, axs = create_figure(ax=ax, n=len(colors), **fig_params)

    for i, c in enumerate(colors):
        ax = axs[i]

        data = np.arange(c.N).reshape(1, -1)

        ax.imshow(data, aspect='auto', cmap=c)

        if labels[i] is not None and len(labels[i]) == c.N:
            ax.set_xticks(np.arange(len(labels[i])))
            ax.set_xticklabels(labels[i], size='small')
        else:
            ax.set_xticks([])
        ...
        if c.name is not None:
            ax.set_title(c.name, size='small', loc='left')

    set_title(plt_params['title'])
    set_note(plt_params['note'])
...
def plot_heatmap_grid(data, show=True, path=None, **kwargs):
    _plt_params = dict(
        cmap='gray', vmin=None, vmax=None, cbar=False, title=None, note=None
    )
    _fig_params = dict(
        n_cols=2, figsize=None
    )
    ...
    if isinstance(data, (list, tuple, np.ndarray)):
        data = {str(i): img for i, img in enumerate(data)}
    ...
    for i, item in enumerate(items):
        item_plt_params = dict(plt_params) | dict(cbar=False, title=item, note=None)
        item_fig_params = dict(fig_params) | dict(figsize=None)
    ...
    if plt_params['cbar']:
        fig.colorbar(axs[0].get_children()[0], ax=axs, pad=0.01, location='right')
...
def plot_cmap(cmap, show=True, path=None, **kwargs):
    _plt_params = dict(
        cell_width=300, cell_height=20, swatch_width=50, margin=10, dpi=72,
        n_cols=4, title=None, note=None
    )
    ...
    cell_width = plt_params['cell_width']
    cell_height = plt_params['cell_height']
    swatch_width = plt_params['swatch_width']
    margin = plt_params['margin']
    dpi = plt_params['dpi']
    n_cols = plt_params['n_cols']
    n_rows = int(np.ceil(n_items / n_cols))

    width = cell_width * n_cols + 2 * margin
    height = cell_height * n_rows + 2 * margin
    figsize = (width / dpi, height / dpi)

    fig, ax = create_figure(figsize=figsize, dpi=dpi)
    ...
    for i, item in enumerate(items):
        ...
        color = cmap[item]
        if isinstance(color, (list, tuple)) and any(np.array(color) > 1):
            color = tuple(np.array(color) / 255)
        color = get_color(color, mode='rgba')[:3]

        ax.text(
            text_pos_x, y, item, fontsize=14, horizontalalignment='left', verticalalignment='center'
        )

        ax.add_patch(
            mpatches.Rectangle(xy=(swatch_start_x, y - 9), width=swatch_width, height=18, facecolor=color, edgecolor='0.7')
        )
