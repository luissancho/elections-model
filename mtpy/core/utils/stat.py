...
class StatMeta(type):
    def __getattribute__(cls, name):
        ...
        if isinstance(attr, FunctionType) and getattr(attr, '_pstatic', False):
            cls_params = list(inspect.signature(cls.__init__).parameters.keys())[1:]
            attr_params = list(inspect.signature(attr).parameters.keys())[1:]

            def wrapper(*args, **kwargs):
                cls_kwargs = {
                    cls_params[i]: arg for i, arg in enumerate(args) if i < len(cls_params)
                } | {
                    k: v for k, v in kwargs.items() if k in cls_params
                }
                attr_kwargs = {
                    k: v for k, v in kwargs.items() if k in attr_params
                }

                instance = cls(**cls_kwargs)
                attr = getattr(instance, name)(**attr_kwargs)
...
class Stat(metaclass=StatMeta):
    ...
    def set_params(self):
        ...
        self.weights *= self.neff / np.sum(self.weights[self.vind])
        self.dof = self.neff - 1
    ...
    @pstatic
    def min(self) -> float:
        return np.min(self.data[self.vind])
    
    @pstatic
    def max(self) -> float:
        return np.max(self.data[self.vind])

    @pstatic
    def mean(self) -> float:
        return np.sum(self.weights[self.vind] * self.data[self.vind]) / self.neff
    ...
    @pstatic
    def var(self) -> float:
        return np.sum(self.weights[self.vind] * np.square(self.data[self.vind] - self.mean())) / self.dof
    ...
    def std_mean(self) -> float:
        return self.std() / np.sqrt(self.neff)
    ...
    def scale_minmax(self, vmin=0, vmax=1):
        self.data = (self.data - self.min()) / (self.max() - self.min()) * (vmax - vmin) + vmin

        return self

    @pstatic
    def scale_sample(self):
        self.data = self.data * self.neff / np.sum(self.data)

        return self

    @pstatic
    def scale_weights(self):
        self.data *= (np.square(np.sum(self.data)) / np.sum(np.square(self.data))) / np.sum(self.data)
...
class Kernel(object):
    ...
    def __init__(
        ...
        if self.weights is not None:
            ...
            self.weights *= self.neff / self.weights.sum()
        else:
    ...
    @staticmethod
    def isj_root(fn, n, args, maxiter=100):
        n = max(min(1050, n), 50)
        bw = 0.
        ...
        while not found and iter < maxiter:
            try:
                bw, res = brentq(fn, lb, rb, args=args, full_output=True, maxiter=maxiter, disp=False)
                found = res.converged
            except ValueError:
                bw = 0.
                rb *= 2.
        ...
        for i in np.arange(self.kvar):
            ...
            if is_timeseries(self.exog[:, i]):
                ...
                if self.ts_freq is None:
                    self.ts_freq = Freq.infer(self.exog[:, i], default=Freq.DAY)
...
class LocalKernelEstimator(Estimator):
    ...
    def predict(
        ...
        if alpha is not None:
            result = pd.DataFrame(
                columns=['mean', 'cmin', 'cmax', 'err', 'nobs', 'neff'],
...
class KernelDensityEstimator(Estimator):
    ...
    def build_input(
        ...
        _, self.kvar = array_shape(self.data)
