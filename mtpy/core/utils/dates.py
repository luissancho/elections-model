...
class Freq(str, Enum):
    ...
    @staticmethod
    def parse(freq: str, default: Optional[str] = None, errors: Literal['ignore', 'raise', 'coerce'] = 'raise') -> str:
        """
        ...
        errors : {'ignore', 'raise', 'coerce'}, default 'raise'
            If 'ignore', then invalid parsing will return `None`.
            If 'raise', then invalid parsing will raise an exception.
            If 'coerce', then invalid parsing will return the default value.
    ...
    @staticmethod
    def infer(x: Any, default: Optional[str] = None, errors: Literal['ignore', 'raise', 'coerce'] = 'raise') -> str:
        """
        ...
        if is_string(x):
            return Freq.parse(x, default=default, errors=errors)

        # Already a datetime index -> infer freq
        if is_timeindex(x):
            if len(x) > 2:  # We need at least 3 dates in order to infer freq
                return Freq.parse(x.inferred_freq, default=default, errors=errors)
            else:
        ...
        if is_array(x):
            ...
            if is_pandas(x):
                ...
                if is_timeindex(x.index):
                    if len(x.index) > 2:  # We need at least 3 dates in order to infer freq
                        return Freq.parse(x.index.inferred_freq, default=default, errors=errors)
                    else:
                        return Freq.parse(default, errors=errors)
                else:
            ...
            if len(dt_seq) > 2:  # We need at least 3 dates in order to infer freq
                return Freq.parse(pd.DatetimeIndex(dt_seq).inferred_freq, default=default, errors=errors)
